    ...
    public void runPendingAnimations() {
        ...
        if (!mPendingAdd.isEmpty()) {
            for (final RecyclerView.ViewHolder viewHolder : mPendingAdd) {
                ...
                ViewHelper.setPivotX(target,target.getMeasuredWidth() / 2);
                ViewHelper.setPivotY(target,target.getMeasuredHeight() / 2);
        ...
        if (!mPendingRemove.isEmpty()) {
            for (final RecyclerView.ViewHolder viewHolder : mPendingRemove) {
                ...
                animator.playTogether(
                        ...
                        ObjectAnimator.ofFloat(target, "alpha", ViewHelper.getAlpha(target), 0.0f)
                );
